...
日本語テキストを含むスライドでは、以下の禁則処理ルールを適用すること。

#### 行頭禁則文字（行の先頭に配置してはいけない文字）
...
句読点（`、` `。` `,` `.`）が行末に来る場合：
- テキストボックスの右端を若干超えてもOK（最大1文字分）
- これにより前の文字を次行に送らず、読みやすさを保つ

#### 約物（記号）の半角スペース処理

**欧文との混在時**
- 欧文と日本語の間：半角スペース不要（日本語組版では通常入れない）
- 括弧内の欧文：`（AWS）` のように括弧と欧文の間にスペースなし

**例外：技術文書での慣例**
- 英単語と日本語の間に半角スペースを入れる場合もある（プロジェクトの方針に従う）

#### 実装上の注意

**PowerPoint / pptxgenjs**
- PowerPointは自動的に基本的な禁則処理を適用する（日本語フォント使用時）
- 手動でテキストを配置する場合は上記ルールを意識する
- テキストボックスの幅を適切に設定し、自然な改行位置を確保

**python-pptx**
```python
# 日本語テキストの場合、フォントを明示的に設定
from pptx.util import Pt
text_frame.paragraphs[0].font.name = 'ＭＳ Ｐゴシック'
...
```xml
<a:pPr>
  <a:defRPr lang="ja-JP" sz="1400" b="0">
...
- [ ] 行頭に句読点や閉じ括弧が来ていないか
- [ ] 行末に開き括弧が来ていないか
- [ ] 三点リーダーやダッシュが分割されていないか（`…` と `…` が別行など）
- [ ] 小書き仮名（っ、ゃ、ょなど）が行頭に来ていないか
- [ ] テキストボックスの幅が狭すぎて不自然な改行が発生していないか
- [ ] 長い英単語やURLが適切な位置で改行されているか
