...
**英単語**
- 英単語は途中で改行しない（例：`PowerPoint` を `Power-` と `Point` に分割しない）
- ハイフン付き英単語（例：`real-time`）もハイフン以外の位置で分割しない

**数値表現**
- 組み数字：`1,000` `10,000,000`
- 連数字：`123456` `2024`
- 日付：`2024年3月22日` （年月日単位で分割しない）
- 時刻：`10:30` `14:00:00`
- 単位付き数値：`100kg` `3.5GB` `120km/h`

**ルビ（ふりがな）付きテキスト**
- グループルビは分離禁止（例：「大人（おとな）」）
- 熟字訓も分離禁止（例：「明日」→「あした」）

**URL・メールアドレス**
- `https://example.com/path/to/page`
- `user@example.com`
- 長い場合はスラッシュ `/` やハイフン `-` の位置で改行可（行頭禁則を守る）
...
- 字間を広げて、禁則文字を次の行に送る
- 前の行の文字間隔が均等に広がる
- 使用例：行が短い場合や、詰めると読みにくくなる場合

**追い込み（押し込み）**
- 字間を詰めて、禁則文字を前の行に収める
- 前の行の文字間隔が均等に縮まる
- 使用例：行が長い場合や、広げるとスカスカになる場合

**PowerPointでの実装**
- PowerPointは自動的に追い込み処理を優先
- 手動調整が必要な場合はテキストボックスの幅を調整
- 極端に詰まる/広がる場合は改行位置を手動で調整
...
**基本禁則**
- [ ] 行頭に句読点（`、` `。`）や閉じ括弧（`」` `）`）が来ていないか
- [ ] 行頭にハイフン（`‐` `–`）や中点（`・` `：`）が来ていないか
- [ ] 行末に開き括弧（`「` `（`）が来ていないか
- [ ] 小書き仮名（`っ` `ゃ` `ゅ` `ょ`など）が行頭に来ていないか

**分離禁止**
- [ ] 三点リーダー（`……`）やダッシュ（`——`）が分割されていないか
- [ ] 英単語が途中で改行されていないか（例：`Power` と `Point` に分割）
- [ ] 数値表現（`1,000` `2024年3月`）が途中で改行されていないか
- [ ] URL（`https://example.com`）が不適切な位置で改行されていないか

**レイアウト調整**
- [ ] テキストボックスの幅が狭すぎて不自然な改行が発生していないか
- [ ] 追い込み・追い出し処理で極端に詰まったり広がったりしていないか
- [ ] ぶら下がり処理が適切に働いているか（句読点が自然に収まっているか）
- [ ] 長い文章が複数行にまたがる場合、読みやすい位置で改行されているか
