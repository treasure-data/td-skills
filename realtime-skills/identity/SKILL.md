...
Identity logging on the event-processing path is enabled for every instance and requires no opt-in. Logging on the personalization request path is off by default because it adds latency to a latency-sensitive path. Customer Support enables it per instance when needed for an investigation.
