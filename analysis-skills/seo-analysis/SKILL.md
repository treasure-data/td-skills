...
- Write grid-dashboard YAML **page by page** (one page at a time to avoid context exhaustion)
...
Write analysis results as a **paged grid-dashboard YAML** and call `preview_grid_dashboard`. See the **grid-dashboard** skill for full cell type and YAML structure reference.

### CRITICAL: Build YAML Page by Page

The dashboard YAML is **one file per site** with analyzed pages as keys under `pages:`. Each page produces a 4×16 grid — this is too large to write in a single pass.

**Workflow**:
1. Write `pages:` header + first page's complete grid/cells to the YAML file
2. Call `preview_grid_dashboard` to verify rendering
3. Append the next page to the same YAML file
4. Repeat until all pages are written, then call `preview_grid_dashboard` again

### Page Grid Layout (4×16 per page)

Each page under `pages:` uses a **4-column × 16-row grid**:
...
pages:
    ...
    cells:
      ...
      - pos: ["9-1", "9-4"]
        ...
        table:
          ...
          rows:
            - ["what is cdp", "/blog/cdp-guide", 11.2, "/products/cdp", 18.7, "Consolidate to /blog/cdp-guide"]
      ...
      - pos: ["11-1", "11-4"]
        ...
        table:
          ...
          rows:
            - ["/blog/data-integration", "customer data platform", "/blog/cdp-guide", "Strong page linking to target"]
      ...
      - pos: ["12-1", "13-4"]
        ...
        content: |
          ...
          **Impact**: High | **Dimension**: Content Structure | **Location**: H2 section #3
          ...
          > **After**: "A CDP works by collecting first-party customer data from websites, apps, and offline sources..."
      ...
      # ── Row 14: Recommended Content Outline ──────────────────────────
      - pos: ["14-1", "14-4"]
        type: markdown
        title: "Recommended Content Outline"
        content: |
          Based on top 5 SERP winners (avg 2,880 words, 10.6 headings, guide format):

          1. **H1: What is a CDP? The Complete Guide (2026)**
          2. **H2: TL;DR** *(BLUF Pattern 1: definition-first, ≤60 words)*
          3. **H2: How Does a CDP Work?** *(BLUF Pattern 1 — matches Answer Box)*
          4. **H2: CDP vs DMP vs CRM** *(BLUF Pattern 3: verdict-first)*
          5. **H2: How Much Does a CDP Cost?** *(BLUF Pattern 2: number-first)*
          6. **H2: Do You Need a CDP?** *(BLUF Pattern 5: yes/no-first)*
          7. **H2: How to Implement a CDP** *(BLUF Pattern 4: step-first)*
          8. **H2: FAQ** *(4 PAA questions as H3s — FAQPage schema target)*

          **Target**: ~3,000 words | 8+ H2 | 10+ images/charts | FAQPage + Article + Organization schema
      ...
      - pos: ["15-1", "15-4"]
        ...
        content: |
          **Expand (Strong)**: CDP cluster at 78% page-1 rate. Create subtopic pages for long-tail.
          **Prioritize (Emerging)**: Data integration at 25%. Create pillar page + internal links.
          **Evaluate (Weak)**: Customer analytics at 13%. Assess before investing.
      ...
      - pos: ["16-1", "16-4"]
        ...
        content: |
          **Metrics**: GSC CTR, GA4 engagement, position for target keywords, cannibalization, link click-through.
          **Timeline**: Title/meta: 2-4 weeks. Content: 4-8 weeks. Schema: 2-6 weeks.
          **Review cadence**: Re-run every **2-3 months**. Track trends. If stalled, update content.

  # ── Next page would follow here ──────────────────────────────────────
  "https://example.com/products/cdp":
    ...
    grid:
      ...
      rows: 16
    cells:
      # ... same 16-row structure as above
```
...
The dashboard renders a **select box** at the top to switch between analyzed pages.

**SERP features column convention**: `AB` = Answer Box, `AI` = AI Overview, `PAA` = People Also Ask, `KG` = Knowledge Graph, `LP` = Local Pack, `SH` = Shopping.

**Drift column convention**: `Stable (0)`, `Rising (-2)`, `Declining (+4)`, `Crash (+8)`, `Surge (-6)`.
